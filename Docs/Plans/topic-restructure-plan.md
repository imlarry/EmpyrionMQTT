...
```
EMP/+/{connId}/+/Req/#                    -- misses 8-segment device topics
EMP/+/{connId}/Structure/Device/+/Req/#   -- required second subscription
```
...
**Event:**
```
EMP/{participantType}/{connectionId}/Evt/{scope}/{eventName}
```

**Response / Error:**
```
EMP/{participantType}/{connectionId}/Res/{scope}/{cid}
EMP/{participantType}/{connectionId}/Err/{scope}/{cid}
```

**Log:**
```
EMP/{participantType}/{connectionId}/Log/App/{level}
```
...
```
EMP/+/{connId}/Evt/#           -- all events from this participant
EMP/Pfs/+/Evt/Player/#         -- all player events across all Pfs
EMP/+/+/Req/Structure/#        -- all structure requests from any participant
EMP/+/+/Log/#                  -- all log messages
```
...
- `MqttRequester` uses a legacy `Client/Q/...` schema entirely separate from EMP/ -- not affected.
...
- `LogAsync`: topic becomes `EMP/Messenger/{clientId}/Log/App/{operation}`.
- Will topic: `EMP/Registry/{clientId}` (prefix only; no structural change).

### ESB (server)

**BusService/SubscriptionHandler.cs**
- Replace two `SubscribeBrokerAsync` calls with one: `$"EMP/+/{connId}/Req/#"`.

**BusService/BusManager.cs**
- `ESB/Registry/` -> `EMP/Registry/` in `PublishRegistryEntryAsync` and `ClearRegistryEntryAsync`.

**EventHandlers/HandlerBase.cs**
- `EmitEventAsync`: `$"ESB/{type}/{connId}/{scope}/Evt/{eventName}"` -> `$"EMP/{type}/{connId}/Evt/{scope}/{eventName}"`.
- `Execute` error topic: `$"ESB/{type}/{connId}/App/Err/{...}"` -> `$"EMP/{type}/{connId}/Err/App/{...}"`.
...
- Hardcoded log topics: `ESB/Agent/{connId}/App/Log/LuaMqttApi.*` -> `EMP/Agent/{connId}/Log/App/LuaMqttApi.*`.
...
- Rename prefix `ESB/` -> `EMP/` in all examples (the format spec already uses `EMP/`; examples were inconsistent).
