...
{
    public class EmpPlayer
    {
        private readonly ContextData _ctx;

        public EmpPlayer(ContextData ctx) { _ctx = ctx; }
        ...
        {
            _ctx.Messenger.RegisterEmpHandler("Player/Req/get",              Properties);
            _ctx.Messenger.RegisterEmpHandler("Player/Req/call/Teleport",    Teleport);
            _ctx.Messenger.RegisterEmpHandler("Player/Req/call/DamageEntity", DamageEntity);
        }

        // Player/Req/get
        //   payload (all optional):
        //     "Properties": ["Health", "Credits", ...]  -- omit for all
        public async Task Properties(EmpMessageContext ctx)
        {
            try
            {
                ...
                if (propArray != null)
                {
                    if (!TryParseMask(propArray, out mask, out var invalid))
                    {
                        ...
                        await EmpHandlerBase.ReplyErrorAsync(_ctx.Messenger, ctx, errObj.ToString(Formatting.None));
                        return;
                    }
                }
        ...
        public async Task Teleport(EmpMessageContext ctx)
        {
            try
            {
                ...
                if (args["Pos"] == null)
                {
                    await EmpHandlerBase.ReplyErrorAsync(_ctx.Messenger, ctx, MessageHelpers.ErrorJson("Pos argument is required"));
                    return;
                }
                if (playfield != null && args["Rot"] == null)
                {
                    await EmpHandlerBase.ReplyErrorAsync(_ctx.Messenger, ctx, MessageHelpers.ErrorJson("Rot argument is required when Playfield is provided"));
                    return;
                }
                ...
                await _ctx.MainThreadRunner.RunOnMainThread(async () =>
                {
                    ...
                    await EmpHandlerBase.ReplyAsync(_ctx.Messenger, ctx,
        ...
        public async Task DamageEntity(EmpMessageContext ctx)
        {
            try
            {
                var lp = _ctx.ModApi.Application.LocalPlayer;
